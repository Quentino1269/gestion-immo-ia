...
| **FRONTEND**      | Implémenter l'écran et la liaison vers le port d'entrée du backend.                                                        | Composants React + appels `fetch` / client API, styles Tailwind, gestion d'erreurs ; canvas Claude Design (skill `design`) tenu à jour pour l'écran. |
...
- Casquette FRONTEND, systématiquement après toute création ou modification d'écran (avant la porte qualité) : lancer le skill `design` pour produire ou mettre à jour le canvas Claude Design (maquette interactive) de l'écran concerné. Le canvas est un livrable de la casquette FRONTEND au même titre que le code.
...
| **Modification d'un Bien Existant**| ✅ **COMPLETED** (2026-09-03) | `docs/slices/modification-bien.md` | Corrige les champs opérationnels (loyer, charges, meublé, disponibilité, libellé chambre, pièces) d'un bien déjà créé. |
| **Suppression d'une Simulation de Rentabilité** | ✅ **COMPLETED** (2026-09-03) | `docs/slices/suppression-simulation-rentabilite.md` | Retire une simulation de rentabilité (append-only : le fait est préservé dans l'event store, seule la projection de lecture disparaît). |
...
- 2026-09-03 — Slice **Modification d'un Bien Existant** modélisé et validé (`docs/slices/modification-bien.md`) : commande composite `ModifierBien` émettant 0..N événements fins selon les champs réellement modifiés (`LoyerRevise`, `ChargesRevisees`, `MeubleEntreDansLeLogement`/`LogementDevenuNu`, `DisponibiliteRevisee`, `LibelleChambreRenomme`, `NombrePiecesRevise`). Périmètre V1 volontairement restreint : `surfaceM2`, `adresse`, `typeBien`, `bienParentId` restent hors scope (risques de cascade sur les invariants de colocation ou changement de nature structurelle), pas d'historique consultable des révisions. Aucun impact rétroactif sur les simulations de rentabilité (elles snapshotent déjà leurs propres valeurs). Conséquence technique anticipée : `Bien` passera de « write-once » à un aggregate rejouable (premier `reconstruire(...)` sur ce bounded context).
- 2026-09-03 — Slice **Modification d'un Bien Existant** implémenté end-to-end (protocole BACKEND → QA → FRONTEND, commits `2bef356`, `43a8798`, `c9d21e2`) : `Bien.reconstruire(...)` (premier rejeu sur ce bounded context), `BienRepository.chargerParId`, `BienService.modifier(...)` (diff candidat/état courant, no-op silencieux si rien ne change), `PUT /api/biens/{id}`, écran `ModifierBienPage` (identité en lecture seule + champs modifiables), canvas Claude Design. 24 tests ajoutés (218 tests backend verts). Porte qualité : code-review (2 correctifs frontend appliqués — validation loyer/charges/pièces, et la case « meublé » qui restait à tort désactivée pour une chambre en colocation alors que ce champ est modifiable par le slice), security-review (clean).
- 2026-09-03 — Trois retours de test personnel de l'utilisateur sur le slice Modification d'un Bien Existant, traités le même jour :
  1. **Restructuration de la création d'une colocation** (révision de D15, `docs/slices/creation-bien.md`) : le type `CHAMBRE_COLOCATION` n'est plus sélectionnable isolément dans « Nouveau bien » — une case à cocher « C'est une colocation » y révèle une saisie groupée des chambres, soumises séquentiellement (composant partagé `ChambreLigneForm`, erreur partielle affichée par ligne sans perdre ce qui a réussi). Ajouter une chambre à un bien déjà existant se fait désormais depuis `ModifierBienPage`. Aucun changement du modèle domaine (toujours des `CreerBien` indépendants).
  2. **Thème sombre** : fond de page passé de `bg-slate-50` à `bg-slate-950` sur tout le frontend, pour contraster avec les formulaires/tableaux restés blancs (fieldsets sans fond explicite corrigés, titres et liens de page éclaircis).
  3. **Suppression d'une simulation de rentabilité** modélisée et implémentée end-to-end (`docs/slices/suppression-simulation-rentabilite.md`) : nouvel événement `SimulationRentabiliteSupprimee`, append-only (rien n'est jamais effacé de l'event store, seule la projection de lecture est retirée — `jpa.deleteById`). Une simulation supprimée redevient introuvable par toute route (comparateur, détail, historique) et refuse toute modification ultérieure (`SimulationSupprimeeException`). Suppression idempotente (no-op si déjà supprimée). Bouton « Supprimer » avec confirmation sur chaque ligne du comparateur. 218+ tests backend verts.
- 2026-09-03 — Second tour de retours sur le même test : bloc « Chambres » remonté juste après « Type de bien » dans « Nouveau bien » ; contraste corrigé sur les titres de `<fieldset>` (fond blanc explicite sur chaque `<legend>`, qui chevauche par défaut la bordure et se retrouvait à cheval sur le fond sombre de la page) ; loyer/charges du bien parent d'une colocation désormais dérivés (somme des chambres saisies) plutôt que saisis manuellement ; `PortefeuillePage` passée en arborescence (chambres en sous-blocs indentés sous leur bien parent, KPI ne comptant plus les chambres séparément). Deux bugs corrigés dans « Mon profil civil » (bounded context `utilisateur`, hors périmètre du slice mais signalés en cours de route et corrigés directement, MISSION §7 « sauf bug évident ») : `Utilisateur.completerProfil` n'émettait `DonneesNaissanceRenseignees` que si date/ville/pays de naissance arrivaient tous les trois dans la même commande (une saisie isolée de la date était silencieusement perdue) ; `ProfilUtilisateur.depuis(...)` regroupait les trois champs sous le seul libellé `dateNaissance` dans `champsManquantsPourBail`, le faisant réapparaître comme manquant après coup. Un message de succès redondant avec le bandeau de statut a aussi été retiré. 228 tests backend verts. PR : https://github.com/Quentino1269/gestion-immo-ia/pull/3
