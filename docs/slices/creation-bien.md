...
| D15 | **Point d'entrée UX unique** : la création d'une chambre passe par le même formulaire « Nouveau bien » que les autres types. L'utilisateur choisit « Chambre en colocation » puis sélectionne son bien parent dans un menu déroulant alimenté par son portefeuille (biens de type `MAISON` ou `APPARTEMENT`). Pas d'écran dédié depuis la fiche du bien parent. |
